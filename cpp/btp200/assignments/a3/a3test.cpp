/* BTP200.051 Assignment 3 Test Main
...
*
*  Version 1.00
*  Built from Version 1.01 a2test.cpp (Feb 14 2005)
*  Last Modified February 22 (CS)
*
...
#define NOPERMN   "/public/btp200/x.txt" // Scratch file for this assignment
#define TESTFILE  "paleo_a.dat"          // Test file #1
#define TESTFILE_ "paleo_b.dat"          // Test file #2
#define TESTSLOPE     0.6354829386
#define TESTINTERCPT  2.5098043930
#define TESTCORREL    0.8771592639
#define TESTSLOPE_    0.6163641121
#define TESTINTERCPT_ 2.6012819552
#define TESTCORREL_   0.8687433073
...
bool DataPointTests(int& testNumber, int& totalPassed, int& totalTested) {
	...
	if (fp) {
		...
		// assignment 3 checks
		if(point.saveToFile(NULL)) {
			failed("saveToFile(NULL) did not return false", ++testNumber, ok, totalTested);
		} else
			passed("saveToFile(NULL)", ++testNumber, totalPassed, totalTested);
		rewind(fp);
		if(!point.saveToFile(fp)) {
			failed("saveToFile(fp) did not return true", ++testNumber, ok, totalTested);
		} else
			passed("saveToFile(fp)", ++testNumber, totalPassed, totalTested);
		fclose(fp);
		fp = fopen(SCRATCH, "r");
		if(point.saveToFile(fp)) {
			failed("saveToFile(fp) did not return false", ++testNumber, ok, totalTested);
		} else
			passed("saveToFile(fp)", ++testNumber, totalPassed, totalTested);
		fclose(fp);
	} else {
...
bool DataSetTests(int& testNumber, int& totalPassed, int& totalTested) {
	...
	if (fp) {
		...
		// Assignment 3 checks
		if(set.saveToFile("")) {
			failed("saveToFile(\"\") did not return false", ++testNumber, ok, totalTested);
		} else
			passed("saveToFile(\"\")", ++testNumber, totalPassed, totalTested);
		if(set.saveToFile(NOPERMN)) {
			sprintf(message, "saveToFile(%s) did not return false", NOPERMN);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			sprintf(message, "saveToFile(%s)", NOPERMN);
			passed(message, ++testNumber, totalPassed, totalTested);
		}
		if(!set.saveToFile(SCRATCH)) {
			sprintf(message, "saveToFile(%s) did not return true", SCRATCH);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			sprintf(message, "saveToFile(%s)", SCRATCH);
			passed(message, ++testNumber, totalPassed, totalTested);
		}

		// Copy Constructor
		DataSet setCopy = set;
		if(setCopy.nPoints() != nr) {
			sprintf(message, "nPoints() did not return %d", nr);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("nPoints()", ++testNumber, totalPassed, totalTested);
		}
		if(setCopy.point(nr).getx() != emptyX) {
			sprintf(message, "getx() did not return %10.2lf", emptyX);
			failed(message, ++testNumber, ok, totalTested);
		} else
			passed("getx()", ++testNumber, totalPassed, totalTested);
		if(setCopy.point(nr).gety() != emptyY) {
			sprintf(message, "gety() did not return %10.2lf", emptyY);
			failed(message, ++testNumber, ok, totalTested);
		} else
			passed("gety()", ++testNumber, totalPassed, totalTested);
		for (int i = 0; i < nr; i++) {
			if(setCopy.point(i).getx() != a[i]) {
				sprintf(message, "getx() did not return %10.2lf", a[i]);
				failed(message, ++testNumber, ok, totalTested);
			} else
				passed("getx()", ++testNumber, totalPassed, totalTested);
			if(setCopy.point(i).gety() != o[i]) {
				sprintf(message, "gety() did not return %10.2lf", o[i]);
				failed(message, ++testNumber, ok, totalTested);
			} else
				passed("gety()", ++testNumber, totalPassed, totalTested);
		}
		if(!doubleEqual(setCopy.slope(), TESTSLOPE)) {
			sprintf(message, "slope() did not return %.2lf", TESTSLOPE);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("slope()", ++testNumber, totalPassed, totalTested);
		}
		if(!doubleEqual(setCopy.intercept(), TESTINTERCPT)) {
			sprintf(message, "intercept() did not return %.2lf", TESTSLOPE);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("intercept()", ++testNumber, totalPassed, totalTested);
		}
		if(!doubleEqual(setCopy.correlation(), TESTCORREL)) {
			sprintf(message, "correlation() did not return %.2lf", TESTSLOPE);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("correlation()", ++testNumber, totalPassed, totalTested);
		...
		// Addition Operator
		DataSet conSet = set + nullSet;
		if(conSet.nPoints() != nr) {
			sprintf(message, "nPoints() did not return %d", nr);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("nPoints()", ++testNumber, totalPassed, totalTested);
		}
		if(conSet.point(nr).getx() != emptyX) {
			sprintf(message, "getx() did not return %10.2lf", emptyX);
			failed(message, ++testNumber, ok, totalTested);
		} else
			passed("getx()", ++testNumber, totalPassed, totalTested);
		if(conSet.point(nr).gety() != emptyY) {
			sprintf(message, "gety() did not return %10.2lf", emptyY);
			failed(message, ++testNumber, ok, totalTested);
		} else
			passed("gety()", ++testNumber, totalPassed, totalTested);
		for (int i = 0; i < nr; i++) {
			if(conSet.point(i).getx() != a[i]) {
				sprintf(message, "getx() did not return %10.2lf", a[i]);
				failed(message, ++testNumber, ok, totalTested);
			} else
				passed("getx()", ++testNumber, totalPassed, totalTested);
			if(conSet.point(i).gety() != o[i]) {
				sprintf(message, "gety() did not return %10.2lf", o[i]);
				failed(message, ++testNumber, ok, totalTested);
			} else
				passed("gety()", ++testNumber, totalPassed, totalTested);
		}
		if(!doubleEqual(conSet.slope(), TESTSLOPE)) {
			sprintf(message, "slope() did not return %.2lf", TESTSLOPE);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("slope()", ++testNumber, totalPassed, totalTested);
		}
		if(!doubleEqual(conSet.intercept(), TESTINTERCPT)) {
			sprintf(message, "intercept() did not return %.2lf", TESTSLOPE);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("intercept()", ++testNumber, totalPassed, totalTested);
		}
		if(!doubleEqual(conSet.correlation(), TESTCORREL)) {
			sprintf(message, "correlation() did not return %.2lf", TESTSLOPE);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("correlation()", ++testNumber, totalPassed, totalTested);
		...
		DataSet setAppend;
		if(!setAppend.loadFromFile(TESTFILE_)) {
			sprintf(message, "loadFromFile(%s) did not return true", TESTFILE_);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			sprintf(message, "loadFromFile(%s)", TESTFILE_);
			passed(message, ++testNumber, totalPassed, totalTested);
		}
		conSet = set + setAppend;
		if(conSet.nPoints() != set.nPoints() + setAppend.nPoints()) {
			sprintf(message, "nPoints() did not return %d", set.nPoints() + setAppend.nPoints());
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("nPoints()", ++testNumber, totalPassed, totalTested);
		}
		if(conSet.point(set.nPoints() + setAppend.nPoints()).getx() != emptyX) {
			sprintf(message, "getx() did not return %10.2lf", emptyX);
			failed(message, ++testNumber, ok, totalTested);
		} else
			passed("getx()", ++testNumber, totalPassed, totalTested);
		if(conSet.point(set.nPoints() + setAppend.nPoints()).gety() != emptyY) {
			sprintf(message, "gety() did not return %10.2lf", emptyY);
			failed(message, ++testNumber, ok, totalTested);
		} else
			passed("gety()", ++testNumber, totalPassed, totalTested);
		if(!doubleEqual(conSet.slope(), TESTSLOPE_)) {
			sprintf(message, "slope() did not return %.2lf", TESTSLOPE);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("slope()", ++testNumber, totalPassed, totalTested);
		}
		if(!doubleEqual(conSet.intercept(), TESTINTERCPT_)) {
			sprintf(message, "intercept() did not return %.2lf", TESTSLOPE);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("intercept()", ++testNumber, totalPassed, totalTested);
		}
		if(!doubleEqual(conSet.correlation(), TESTCORREL_)) {
			sprintf(message, "correlation() did not return %.2lf", TESTSLOPE);
			failed(message, ++testNumber, ok, totalTested);
		} else {
			passed("correlation()", ++testNumber, totalPassed, totalTested);
