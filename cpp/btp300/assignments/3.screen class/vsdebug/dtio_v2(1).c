...
int dtioEdit(char *s, int row, int col, int len, int *ppos) {
	...
	/*printf("%d,|%s|",strlen(s),tempS );*/
	
	if (tempS) { tempS = NULL; free(tempS); }
	/*system("pause");
	printf("test");*/
	return key;
}
...
void append(int k, int cpos, int c, char * ts) {
	...
	temp = (char *) malloc(strlen(ts)+1);
	strcpy(temp,ts);*/
	swap = (char *) malloc(strlen(ts)+2);
	
	/*if (cpos -  c < strlen(ts)) {
		temp[cpos - c] = k;
		for (i = cpos - c; i <strlen(ts); i++) {
			temp[i] = ts[i];
		}
		temp[i] = '\0';
	} else {
