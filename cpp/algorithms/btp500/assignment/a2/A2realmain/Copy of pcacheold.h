...
int PCacheHash<CT>::getElement(int hashValue) {
	...
	//if (hashValue == 4) { cout << numOfBuckets << "-element value = " << element << endl; getchar(); }
	return element;
};
template <class CT>
int PCacheHash<CT>::getIndex(int hashValue) { 
	...
	if (hashValue <= numOfBuckets)
		...
		index = ( hashValue - (getElement(hashValue)+1) ) / numOfBuckets;
	}
	//if (hashValue == 4) { cout << numOfBuckets << "-index value = " << index << endl; getchar(); }
	return index;
};
template <class CT>
...
PCacheHash<CT>::~PCacheHash() {
	...
	cout << PermHash<CT>::size() << endl;
	cout << "cache size " << cachesize() << endl;
	cout << "cache cap " << cachecap() << endl;
	getchar();
	int ctr = 0;
	for (int i = 0; i < cachecap(); i++) {
		index = getIndex(i);
		element = getElement(i);
		/*cout << i << endl;
		cout << cacheEntry[index][element].entry.rec << endl;
		if (i < cachecap() / 4) { getchar(); }*/
		if (PermHash<CT>::search(cacheEntry[index][element].entry.rec.key()))
			ctr++;			
		if (!cacheEntry[index][element].entry.isempty)
			PermHash<CT>::update(cacheEntry[index][element].entry.rec);
		/*if (PermHash<CT>::size() == 19700) {
			cout << i << endl;
			cout << cacheEntry[index][element].entry.rec << endl;
			getchar();
		}*/
	...
	cout << "cache size " << cachesize() << endl;
	cout << "cache cap " << cachecap() << endl;
	cout << "number of duplicate keys in perm hash: " << ctr << endl;
	getchar();
	cout << PermHash<CT>::size() << endl;
...
bool PCacheHash<CT>::update(const CT& data) {
	...
	if(PermHash<CT>::size() < PermHash<CT>::max()) {
		//if (debugger == 108214) cout << "check1" << endl;
		int newHashValue = HashFunction(data.key(),cachecap());
		int i = newHashValue;
		
		//if (i == 0) { cout << "POSSIBLE ZERO VALUE FOR HASHVALUE!!!" << endl; }

		if (i == 3) {
			cout << cacheEntry[getIndex(i)][getElement(i)].entry.rec << endl;
		}

		if (cachesize() == cachecap() - 1) {
			
			if (i == 3) { cout << "1check" << endl; }
			
			//if (debugger == 108214) cout << "check2" << endl;
			// retire to perm table then replace the least oldest entry
			rc = PermHash<CT>::update(cacheEntry[getIndex(oldest)][getElement(oldest)].entry.rec);
			if (rc) { 
				int index = getIndex(oldest);
				int element = getElement(oldest);
			...
			//if (debugger == 108214) cout << "check3" << endl;
			int index = getIndex(i);
			int element = getElement(i);
			if (cacheEntry[index][element].entry.isempty) {

				if (i == 3) { cout << "2check" << endl; }

				//if (debugger == 108214) cout << "check4" << endl;
				cacheEntry[index][element].entry.rec = data;
				...
				if (i == 3) { cout << "3check" << endl; }
				
				//if (debugger == 108214) cout << "1check5" << endl;
				cacheEntry[index][element].entry.rec = data;
				cacheEntry[index][element].entry.isempty = false;
				rc = true;
				//if (debugger == 108214) cout << "2check5" << endl;
			} else {
			// loop stops when hits first empty spot or if it reaches same spot
			// considering that hashtable can loop from current at end to next current is at beg
				//if (debugger == 108214) cout << "check6" << endl;
				if (i == 3) { cout << "3a check - " << newHashValue << endl; }
				i++;
				bool keepgoing = true;
				while (keepgoing) {
					if (i == cachecap()) i = 0;
					if (i == newHashValue) {
						...
						index = getIndex(i);
						element = getElement(i);
						if (i == 3 || newHashValue == 3) cout << i << "_main:" << index << "," << element << endl;
						if (cacheEntry[index][element].entry.isempty) {
							if (i == 3 || newHashValue == 3) { 
								cout << "4.1check" << endl;
								cout << "index for " << i << " = " << getIndex(i) << ", element for " << i << " = " << getElement(i) << endl;
								cout << "index for " << newHashValue << " = " << getIndex(newHashValue) << ", element for " << newHashValue << " = " << getElement(newHashValue) << endl;
								cout << cacheEntry[getIndex(newHashValue)][getElement(newHashValue)].entry.rec << endl;
								cout << "********************************************************************" << endl;
							}
							rc = true;
							keepgoing = false;
							...
							if (i == 3 || newHashValue == 3) { cout << cacheEntry[index][element].entry.rec << endl; getchar(); }
							cacheEntry[index][element].entry.rec = data;
							if (i == 3 || newHashValue == 3) { 
								cout << "4.3check" << endl;
								cout << "index for " << i << " = " << getIndex(i) << ", element for " << i << " = " << getElement(i) << endl;
							...
							if (i == 3 || newHashValue == 3) { cout << "5check" << endl; }
							rc = true;
							keepgoing = false;
							cacheEntry[index][element].entry.isempty = false;
							cacheEntry[index][element].entry.rec = data;
	...
	//if (debugger == 108214) cout << "closer update status " << rc << endl;
	return rc;
};
...
bool PCacheHash<CT>::find(const char* key, CT& holder) {
	/*if (debugger == 108214) {
		cout << "cache size = " << cachesize() << " cachecap = " << cachecap() << endl;
		getchar();
	}
	debugger++;
	cout << debugger << endl;*/
...
bool PCacheHash<CT>::remove(const char* key) {
	...
	if (rm == 3) {
		cout << "in remove" << endl;
		cout << cacheEntry[getIndex(rm)][getElement(rm)].entry.rec << endl;
		getchar();
	}
	...
	if (rm != -1) {
		...
		if (cacheEntry[index][element].entry.isempty) { cout << "it worked @ pos " << rm << endl; }
